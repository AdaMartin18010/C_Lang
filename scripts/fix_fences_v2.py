    ...
    for i, line in enumerate(lines):
        if fence_re.match(line):
            if not in_fence:
                ...
                in_fence = False
    return in_fence, fence_start, lines

def fix_file(filepath):
    in_fence, fence_start, lines = check_unclosed(filepath)
    ...
    # Verify fix
    in_fence, _, _ = check_unclosed(filepath)
    if in_fence:
...
# First restore the files that were partially modified
import shutil
for f in files:
    backup = f + '.backup'
