    ...
    for i, line in enumerate(lines):
        if fence_re.match(line):
            if not in_fence:
                ...
                in_fence = False
    if in_fence:
        print(f'=== {f} (unclosed at line {fence_start}) ===')
        start = max(0, fence_start - 3)
        end = min(len(lines), fence_start + 8)
        for j in range(start, end):
            marker = ' >>>' if j == fence_start - 1 else ''
            print(f'{j+1:4d}{marker}: {lines[j].rstrip()}')
        print('...')
        for j in range(max(fence_start, len(lines)-3), len(lines)):
