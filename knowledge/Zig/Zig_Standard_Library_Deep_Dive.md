# Zig 标准库深度解析：从容器到网络编程

> **定位**: Zig 核心知识 / 标准库精通 | **难度**: ⭐⭐⭐⭐ | **版本**: 0.13+

---

## 📋 目录

- [Zig 标准库深度解析：从容器到网络编程](#zig-标准库深度解析从容器到网络编程)
  - [📋 目录](#-目录)
  - [📑 目录](#-目录-1)
  - [一、内存分配器家族](#一内存分配器家族)
    - [1.1 分配器全景图](#11-分配器全景图)
    - [1.2 分配器实战代码](#12-分配器实战代码)
  - [二、容器类型全景](#二容器类型全景)
    - [2.1 ArrayList - 动态数组](#21-arraylist---动态数组)
    - [2.2 HashMap - 哈希表](#22-hashmap---哈希表)
    - [2.3 其他容器](#23-其他容器)
  - [三、字符串处理](#三字符串处理)
    - [3.1 字符串操作大全](#31-字符串操作大全)
    - [3.2 格式化字符串](#32-格式化字符串)
  - [四、文件系统操作](#四文件系统操作)
    - [4.1 文件读写](#41-文件读写)
    - [4.2 目录操作](#42-目录操作)
    - [4.3 路径操作](#43-路径操作)
  - [五、网络编程](#五网络编程)
    - [5.1 TCP 客户端/服务器](#51-tcp-客户端服务器)
    - [5.2 UDP 编程](#52-udp-编程)
    - [5.3 HTTP 客户端/服务器](#53-http-客户端服务器)
  - [六、加密与安全](#六加密与安全)
    - [6.1 哈希算法](#61-哈希算法)
    - [6.2 HMAC 与密钥派生](#62-hmac-与密钥派生)
    - [6.3 对称加密](#63-对称加密)
  - [七、序列化格式](#七序列化格式)
    - [7.1 JSON 处理](#71-json-处理)
    - [7.2 其他格式](#72-其他格式)
  - [八、进程与线程](#八进程与线程)
  - [九、时间与时钟](#九时间与时钟)
  - [十、调试与日志](#十调试与日志)
...
│  │  alloc, free, realloc, allocAdvanced...                       │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                              ▲                                       │
│          ┌───────────────────┼───────────────────┐                   │
│          ▼                   ▼                   ▼                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐       │
│  │   General    │  │    Arena     │  │     Fixed Buffer      │       │
│  │   Purpose    │  │              │  │                      │       │
│  │ Allocator    │  │  一次性释放   │  │    栈/静态内存分配     │       │
│  │  (GPA)       │  │              │  │                      │       │
│  └──────────────┘  └──────────────┘  └──────────────────────┘       │
│          │                   │                   │                   │
│          ▼                   ▼                   ▼                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐       │
│  │   Page       │  │   CAlloc     │  │     HeapAllocator    │       │
│  │  Allocator   │  │   (malloc)   │  │    (Windows Heap)    │       │
│  │  (mmap)      │  │              │  │                      │       │
│  └──────────────┘  └──────────────┘  └──────────────────────┘       │
│                                                                      │
│  特殊用途:                                                            │
│  - LoggingAllocator: 记录分配统计                                     │
│  - StackFallbackAllocator: 栈优先，不足转堆                           │
│  - SegmentedList: 分段增长，避免重新分配                              │
│  - MutexProtected: 线程安全包装                                       │
└─────────────────────────────────────────────────────────────────────┘
```
