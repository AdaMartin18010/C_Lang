...
本节配套可编译代码位于 `examples/design_patterns/` 目录：

| 示例文件 | 演示内容 | 编译命令 |
|---------|---------|---------|
| `vtable_polymorphism.c` | 虚表实现运行时多态（Shape/Circle/Rectangle） | `gcc -O2 -std=c11` |
| `state_machine.c` | 表驱动有限状态机，函数指针数组实现状态转换 | `gcc -O2 -std=c11` |
| `observer.c` | 观察者模式，多订阅者回调通知 | `gcc -O2 -std=c11` |

### 各模式性能对比

| 模式 | 间接调用开销 | 内存开销 | 扩展性 |
|------|------------|---------|--------|
| 虚表多态 | 2 次指针解引用 | 每个对象 + 全局 vtable | 运行时动态 |
| 表驱动状态机 | 1 次数组索引 + 1 次调用 | O(状态数 × 事件数) | 编译期静态 |
| 观察者模式 | O(n) 遍历 + n 次调用 | O(订阅者数) | 运行时动态 |
...
| 错误模式 | 后果 | 修复方案 |
|---------|------|---------|
| **vtable 函数签名不匹配** | 未定义行为，崩溃 | 使用 `typedef` 统一函数指针类型 |
| **基类结构体对齐不一致** | 内存布局错误，字段偏移错位 | 第一个成员必须是 "基类" 结构体 |
| **回调悬空指针** | 回调时访问已释放的 observer | 注销时清除回调，或使用弱引用模式 |
| **状态机未处理默认分支** | 非法事件导致未定义行为 | 添加 `default` 分支保持当前状态或报错 |
| **构造失败返回部分初始化对象** | 使用未初始化的 vtable 指针 | 分配内存 → 初始化所有字段 → 最后设置 vtable |
| **忘记传递 `user_data`** | 回调无法访问上下文 | 所有回调接口强制包含 `void *user_data` |
