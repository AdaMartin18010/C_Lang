...
| 示例路径 | 演示内容 | 编译命令 |
|---------|---------|---------|
| `examples/bit_manipulation/endianness.c` | 字节序检测与转换（浮点序列化相关） | `gcc -O2 -std=c11` |
| `examples/data_structures/dynamic_array.c` | 泛型动态数组（数值存储容器） | `gcc -O2 -std=c11` |

### 浮点性能对比（x86-64 AVX2）

| 操作 | float (GFLOPS) | double (GFLOPS) | 备注 |
|------|---------------|----------------|------|
| 向量加法 | ~100 | ~50 | 8x float vs 4x double |
| 矩阵乘法 | ~200 | ~100 | 内存带宽限制 |
| `sqrt` | ~10 | ~5 | 硬件指令 |
| `sin`/`cos` | ~2 | ~1 | 库函数，精度优先 |
...
| 错误模式 | 后果 | 修复方案 |
|---------|------|---------|
| **浮点 `==` 比较** | 逻辑错误，条件永远为假 | 使用 `fabs(a-b) < epsilon` |
| **大数累加精度损失** | 结果显著偏差 | Kahan 求和或更高精度中间类型 |
| **整数除法截断** | `1/2 == 0` | 先转换为浮点：`1.0/2` |
| **未初始化的浮点变量** | NaN 传播 | 始终初始化浮点变量 |
| **`-ffast-math` 导致 NaN 静默** | 静默失败，难以调试 | 开发阶段禁用，发布时评估 |
| **字节序忽略** | 跨平台数据解析错误 | 网络/文件数据显式转换 |
