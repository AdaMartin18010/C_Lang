...
- [C2y defer 关键字深度解析](#c2y-defer-关键字深度解析)
  - [目录](#目录)
  - [1. defer概述](#1-defer概述)
    - [1.1 什么是defer](#11-什么是defer)
    - [1.2 设计动机](#12-设计动机)
      - [1.2.1 资源管理问题](#121-资源管理问题)
      - [1.2.2 错误处理复杂性](#122-错误处理复杂性)
      - [1.2.3 defer的解决方案](#123-defer的解决方案)
    - [1.3 与其他语言机制的对比](#13-与其他语言机制的对比)
      - [1.3.1 与C++ RAII的对比](#131-与c-raii的对比)
      - [1.3.2 与Go defer的关系](#132-与go-defer的关系)
      - [1.3.3 与Rust Drop trait的对比](#133-与rust-drop-trait的对比)
      - [1.3.4 与Swift defer的对比](#134-与swift-defer的对比)
  - [2. 语法语义详解](#2-语法语义详解)
    - [2.1 defer语句语法](#21-defer语句语法)
      - [2.1.1 单语句defer](#211-单语句defer)
      - [2.1.2 块形式defer](#212-块形式defer)
    - [2.2 执行时机](#22-执行时机)
      - [2.2.1 正常返回](#221-正常返回)
      - [2.2.2 到达函数末尾](#222-到达函数末尾)
      - [2.2.3 goto跳转](#223-goto跳转)
      - [2.2.4 longjmp跳转](#224-longjmp跳转)
      - [2.2.5 exit和abort](#225-exit和abort)
    - [2.3 LIFO执行顺序](#23-lifo执行顺序)
    - [2.4 作用域规则](#24-作用域规则)
      - [2.4.1 块级作用域](#241-块级作用域)
      - [2.4.2 循环中的defer](#242-循环中的defer)
      - [2.4.3 条件语句中的defer](#243-条件语句中的defer)
    - [2.5 参数求值时机](#25-参数求值时机)
  - [3. 使用场景](#3-使用场景)
    - [3.1 文件资源管理](#31-文件资源管理)
      - [3.1.1 单个文件操作](#311-单个文件操作)
      - [3.1.2 临时文件管理](#312-临时文件管理)
    - [3.2 内存分配释放](#32-内存分配释放)
      - [3.2.1 动态内存管理](#321-动态内存管理)
      - [3.2.2 多级指针管理](#322-多级指针管理)
    - [3.3 锁的获取释放](#33-锁的获取释放)
      - [3.3.1 互斥锁管理](#331-互斥锁管理)
      - [3.3.2 读写锁管理](#332-读写锁管理)
      - [3.3.3 多锁管理](#333-多锁管理)
    - [3.4 错误处理清理](#34-错误处理清理)
      - [3.4.1 分阶段初始化清理](#341-分阶段初始化清理)
      - [3.4.2 错误码传播模式](#342-错误码传播模式)
    - [3.5 日志和审计](#35-日志和审计)
      - [3.5.1 函数进入退出日志](#351-函数进入退出日志)
      - [3.5.2 事务日志](#352-事务日志)
  - [4. 代码示例](#4-代码示例)
    - [4.1 基本用法示例](#41-基本用法示例)
      - [示例1：简单资源管理](#示例1简单资源管理)
      - [示例2：多个defer的执行顺序](#示例2多个defer的执行顺序)
      - [示例3：条件返回](#示例3条件返回)
    - [4.2 复杂资源管理示例](#42-复杂资源管理示例)
      - [示例4：网络连接管理](#示例4网络连接管理)
      - [示例5：数据库操作封装](#示例5数据库操作封装)
    - [4.3 错误处理模式](#43-错误处理模式)
      - [示例6：错误处理最佳实践](#示例6错误处理最佳实践)
    - [4.4 与goto对比](#44-与goto对比)
      - [示例7：传统goto vs defer](#示例7传统goto-vs-defer)
    - [4.5 嵌套defer](#45-嵌套defer)
      - [示例8：嵌套函数和defer](#示例8嵌套函数和defer)
      - [示例9：复杂嵌套场景](#示例9复杂嵌套场景)
    - [4.6 高级模式](#46-高级模式)
      - [示例10：作用域守卫模式](#示例10作用域守卫模式)
  - [5. 实现原理](#5-实现原理)
    - [5.1 编译器实现概述](#51-编译器实现概述)
      - [5.1.1 基本转换策略](#511-基本转换策略)
      - [5.1.2 defer栈机制](#512-defer栈机制)
    - [5.2 与setjmp/longjmp的关系](#52-与setjmplongjmp的关系)
      - [5.2.1 传统setjmp/longjmp的问题](#521-传统setjmplongjmp的问题)
      - [5.2.2 C2y defer的解决方案](#522-c2y-defer的解决方案)
      - [5.2.3 实现细节](#523-实现细节)
    - [5.3 性能开销分析](#53-性能开销分析)
      - [5.3.1 运行时开销](#531-运行时开销)
      - [5.3.2 优化策略](#532-优化策略)
      - [5.3.3 与其他语言机制的性能对比](#533-与其他语言机制的性能对比)
    - [5.4 栈展开机制](#54-栈展开机制)
      - [5.4.1 正常情况下的栈展开](#541-正常情况下的栈展开)
      - [5.4.2 异常路径下的栈展开](#542-异常路径下的栈展开)
      - [5.4.3 实现架构](#543-实现架构)
    - [5.5 编译器具体实现案例](#55-编译器具体实现案例)
      - [5.5.1 Clang的实现策略](#551-clang的实现策略)
      - [5.5.2 GCC的实现策略](#552-gcc的实现策略)
  - [6. 与其他语言对比](#6-与其他语言对比)
    - [6.1 语法对比](#61-语法对比)
      - [6.1.1 各语言defer语法汇总](#611-各语言defer语法汇总)
      - [6.1.2 详细语法对比](#612-详细语法对比)
    - [6.2 语义差异](#62-语义差异)
      - [6.2.1 参数求值时机](#621-参数求值时机)
      - [6.2.2 执行顺序](#622-执行顺序)
      - [6.2.3 作用域规则](#623-作用域规则)
    - [6.3 使用模式对比](#63-使用模式对比)
      - [6.3.1 文件操作模式](#631-文件操作模式)
      - [6.3.2 锁管理模式](#632-锁管理模式)
    - [6.4 性能对比](#64-性能对比)
      - [6.4.1 基准测试结果](#641-基准测试结果)
      - [6.4.2 性能影响因素](#642-性能影响因素)
    - [6.5 优缺点对比](#65-优缺点对比)
      - [6.5.1 各语言机制的优点](#651-各语言机制的优点)
      - [6.5.2 选择建议](#652-选择建议)
  - [7. 最佳实践](#7-最佳实践)
    - [7.1 何时使用defer](#71-何时使用defer)
      - [7.1.1 推荐使用defer的场景](#711-推荐使用defer的场景)
      - [7.1.2 不适合使用defer的场景](#712-不适合使用defer的场景)
    - [7.2 避免滥用](#72-避免滥用)
      - [7.2.1 defer使用准则](#721-defer使用准则)
      - [7.2.2 常见反模式](#722-常见反模式)
    - [7.3 性能考虑](#73-性能考虑)
      - [7.3.1 编译器优化建议](#731-编译器优化建议)
      - [7.3.2 性能测试方法](#732-性能测试方法)
    - [7.4 调试技巧](#74-调试技巧)
      - [7.4.1 追踪defer执行](#741-追踪defer执行)
      - [7.4.2 检测资源泄漏](#742-检测资源泄漏)
      - [7.4.3 常见调试场景](#743-常见调试场景)
  - [🔷 GCC 15 支持状态](#-gcc-15-支持状态)
    - [版本信息](#版本信息)
    - [编译选项](#编译选项)
    - [功能支持详情](#功能支持详情)
    - [与GCC扩展的互操作](#与gcc扩展的互操作)
    - [已知问题与限制](#已知问题与限制)
    - [与其他编译器对比](#与其他编译器对比)
  - [8. 编译器支持](#8-编译器支持)
    - [8.1 GCC支持](#81-gcc支持)
      - [8.1.1 版本要求](#811-版本要求)
      - [8.1.2 使用方法](#812-使用方法)
    - [8.2 Clang支持](#82-clang支持)
      - [8.1.1 版本要求](#811-版本要求-1)
      - [8.1.2 使用方法](#812-使用方法-1)
      - [8.1.3 编译示例](#813-编译示例)
    - [8.2 GCC支持](#82-gcc支持)
      - [8.2.1 版本要求](#821-版本要求)
      - [8.2.2 使用方法](#822-使用方法)
    - [8.3 特性检测](#83-特性检测)
      - [8.3.1 预处理器检测](#831-预处理器检测)
      - [8.3.2 CMake检测](#832-cmake检测)
    - [8.4 迁移指南](#84-迁移指南)
      - [8.4.1 从传统C代码迁移](#841-从传统c代码迁移)
      - [8.4.2 与现有代码集成](#842-与现有代码集成)
  - [9. 附录](#9-附录)
    - [9.1 完整示例代码](#91-完整示例代码)
      - [9.1.1 综合示例：配置文件解析器](#911-综合示例配置文件解析器)
    - [9.2 常见问题解答](#92-常见问题解答)
      - [Q1: defer可以取消吗？](#q1-defer可以取消吗)
      - [Q2: defer支持返回值吗？](#q2-defer支持返回值吗)
      - [Q3: 在信号处理程序中可以使用defer吗？](#q3-在信号处理程序中可以使用defer吗)
      - [Q4: defer和线程安全](#q4-defer和线程安全)
    - [9.3 参考资源](#93-参考资源)
      - [9.3.1 官方文档](#931-官方文档)
      - [9.3.2 参考实现](#932-参考实现)
      - [9.3.3 相关论文](#933-相关论文)
