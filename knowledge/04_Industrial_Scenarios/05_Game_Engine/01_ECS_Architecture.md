...
- [游戏引擎ECS架构 - 工业级深度解析](#游戏引擎ecs架构---工业级深度解析)
  - [📋 本节概要](#-本节概要)
  - [📑 目录](#-目录)
  - [1. 概念定义](#1-概念定义)
    - [1.1 ECS的严格定义](#11-ecs的严格定义)
      - [核心三元组](#核心三元组)
    - [1.2 数据导向设计（DOD）原则](#12-数据导向设计dod原则)
      - [DOD vs OOP 对比](#dod-vs-oop-对比)
      - [DOD三原则](#dod三原则)
    - [1.3 面向对象 vs ECS 对比](#13-面向对象-vs-ecs-对比)
    - [1.4 缓存友好性原理](#14-缓存友好性原理)
      - [CPU缓存层次结构](#cpu缓存层次结构)
      - [缓存行与数据布局](#缓存行与数据布局)
  - [2. 属性维度矩阵](#2-属性维度矩阵)
    - [2.1 ECS架构对比](#21-ecs架构对比)
    - [2.2 组件存储策略对比](#22-组件存储策略对比)
    - [2.3 系统调度策略对比](#23-系统调度策略对比)
    - [2.4 内存布局对比](#24-内存布局对比)
    - [2.5 性能特征矩阵](#25-性能特征矩阵)
  - [3. 形式化描述](#3-形式化描述)
    - [3.1 ECS的数学模型](#31-ecs的数学模型)
    - [3.2 组件查询的形式化](#32-组件查询的形式化)
    - [3.3 系统依赖图](#33-系统依赖图)
  - [4. C语言实现](#4-c语言实现)
    - [4.1 完整ECS框架实现](#41-完整ecs框架实现)
    - [4.2 完整游戏示例：2D太空射击游戏](#42-完整游戏示例2d太空射击游戏)
  - [5. 反例/陷阱](#5-反例陷阱)
    - [5.1 陷阱 01: 缓存未命中](#51-陷阱-01-缓存未命中)
    - [5.2 陷阱 02: 虚函数滥用](#52-陷阱-02-虚函数滥用)
    - [5.3 陷阱 03: 指针追逐](#53-陷阱-03-指针追逐)
    - [5.4 陷阱 04: 过度泛化](#54-陷阱-04-过度泛化)
    - [5.5 陷阱 05: 内存碎片](#55-陷阱-05-内存碎片)
    - [5.6 陷阱 06: 同步开销](#56-陷阱-06-同步开销)
    - [5.7 陷阱 07: 数据竞争](#57-陷阱-07-数据竞争)
    - [5.8 陷阱 08: 组件依赖混乱](#58-陷阱-08-组件依赖混乱)
    - [5.9 陷阱 09: 系统顺序错误](#59-陷阱-09-系统顺序错误)
    - [5.10 陷阱 10: 内存泄漏](#510-陷阱-10-内存泄漏)
  - [6. 性能优化](#6-性能优化)
    - [6.1 缓存优化技巧](#61-缓存优化技巧)
    - [6.2 SIMD应用](#62-simd应用)
    - [6.3 多线程并行](#63-多线程并行)
    - [6.4 内存池使用](#64-内存池使用)
    - [6.5 批处理策略](#65-批处理策略)
  - [7. 思维导图](#7-思维导图)
    - [7.1 ECS架构图](#71-ecs架构图)
    - [7.2 数据流图](#72-数据流图)
    - [7.3 系统依赖图](#73-系统依赖图)
  - [✅ 质量验收清单](#-质量验收清单)
  - [深入理解](#深入理解)
