...
- [无锁队列与LMAX Disruptor](#无锁队列与lmax-disruptor)
  - [📑 目录](#-目录)
  - [1. 无锁数据结构原理](#1-无锁数据结构原理)
    - [1.1 为什么需要无锁队列](#11-为什么需要无锁队列)
    - [1.2 内存序（Memory Ordering）](#12-内存序memory-ordering)
  - [2. C11原子操作实现](#2-c11原子操作实现)
    - [2.1 基础原子类型定义](#21-基础原子类型定义)
  - [3. 单生产者单消费者（SPSC）队列](#3-单生产者单消费者spsc队列)
  - [4. 多生产者多消费者（MPMC）队列](#4-多生产者多消费者mpmc队列)
  - [5. LMAX Disruptor模式实现](#5-lmax-disruptor模式实现)
  - [6. 性能测试与对比](#6-性能测试与对比)
  - [7. 性能对比总结](#7-性能对比总结)
  - [深入理解](#深入理解)
    - [核心原理](#核心原理)
