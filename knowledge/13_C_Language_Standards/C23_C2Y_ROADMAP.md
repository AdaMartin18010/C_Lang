...
| MSVC 2022 (17.0) | 部分 | 进行中 | 属性（C++兼容） |
| MSVC 2022 (17.4+) | 较多 | 进行中 | `auto`, `typeof`（C++兼容） |
| MSVC 2022 (17.8+) | 部分 | 进行中 | 无C23专项增强 |
| MSVC 2022 (17.12+) | 部分 | 停滞 | 仍无`_BitInt`/`nullptr`/`#embed` |

**⚠️ 重要警示：** MSVC 截至2026年5月**无官方C23路线图**，`_BitInt`、`nullptr`、`#embed` 等核心特性仍不可用，仅通过 `/std:clatest` 提供有限支持。跨平台C23项目建议以GCC/Clang为主编译器。
...
> **C2y** = ISO/IEC 9899:202Y (预计2028-2029年发布，可能称为C29)
...
| 属性 | 详情 |
|------|------|
| **提案编号** | N3199, N3242, N3734 (TS CD r5), N3853 |
| **状态** | ✅ C2y TS CD r5 (2026-03) |
...
| 属性 | 详情 |
|------|------|
| **提案编号** | N3298, N3309, N3833 r4 (隐式constexpr) |
| **状态** | 🔄 讨论中（函数级）；N3833 r4已提交（ICE初始化隐式constexpr） |
| **动机** | 允许函数在编译期执行；const整型ICE初始化自动获得constexpr语义 |
| **影响** | 重大 - 需要ABI考虑 |
...
| 编译器 | 状态 | 备注 |
|--------|------|------|
| GCC | 🔄 函数级计划中；✅ N3833已部分实现 | GCC 16支持隐式constexpr |
...
/* defer 兼容性 (C2y+) */
#if IS_C2Y
    #define C2Y_DEFER(stmt) defer stmt  /* C2y特性，非C23 */
