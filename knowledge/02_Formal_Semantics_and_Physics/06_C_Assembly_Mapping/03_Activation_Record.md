...
- [📖 核心概念详解](#-核心概念详解)
  - [1. 调用栈基础](#1-调用栈基础)
    - [1.1 栈布局](#11-栈布局)
    - [1.2 活动记录结构](#12-活动记录结构)
  - [2. 调用约定详解](#2-调用约定详解)
    - [2.1 System V AMD64 ABI (Linux/macOS)](#21-system-v-amd64-abi-linuxmacos)
    - [2.2 Windows x64调用约定](#22-windows-x64调用约定)
  - [3. 函数序言和尾声](#3-函数序言和尾声)
    - [3.1 标准序言/尾声](#31-标准序言尾声)
    - [3.2 叶函数优化](#32-叶函数优化)
  - [4. 参数传递机制](#4-参数传递机制)
    - [4.1 寄存器参数](#41-寄存器参数)
    - [4.2 栈参数访问](#42-栈参数访问)
  - [5. 局部变量和栈分配](#5-局部变量和栈分配)
    - [5.1 固定大小局部变量](#51-固定大小局部变量)
    - [5.2 变长数组(VLA)](#52-变长数组vla)
    - [5.3 alloca实现](#53-alloca实现)
  - [6. 栈展开和异常处理](#6-栈展开和异常处理)
    - [6.1 DWARF展开信息](#61-dwarf展开信息)
    - [6.2 栈金丝雀](#62-栈金丝雀)
- [⚠️ 常见陷阱](#️-常见陷阱)
  - [陷阱 AR01: 栈未对齐](#陷阱-ar01-栈未对齐)
  - [陷阱 AR02: 忘记恢复寄存器](#陷阱-ar02-忘记恢复寄存器)
  - [陷阱 AR03: 缓冲区溢出覆盖返回地址](#陷阱-ar03-缓冲区溢出覆盖返回地址)
- [✅ 质量验收清单](#-质量验收清单)
  - [6.3 尾调用优化](#63-尾调用优化)
  - [6.4 栈帧大小的确定](#64-栈帧大小的确定)
- [深入理解](#深入理解)
  - [核心原理](#核心原理)
  - [实践应用](#实践应用)
  - [最佳实践](#最佳实践)
