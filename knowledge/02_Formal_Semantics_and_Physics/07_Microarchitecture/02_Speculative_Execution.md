...
- [推测执行与UB](#推测执行与ub)
  ...
  - [📖 核心概念详解](#-核心概念详解)
    - [1. 推测执行基础](#1-推测执行基础)
      - [1.1 分支预测](#11-分支预测)
      - [1.2 推测执行机制](#12-推测执行机制)
    - [2. Spectre漏洞家族](#2-spectre漏洞家族)
      - [2.1 Spectre V1 (边界检查绕过)](#21-spectre-v1-边界检查绕过)
      - [2.2 Spectre V1 防护措施](#22-spectre-v1-防护措施)
      - [2.3 Spectre V2 (分支目标注入)](#23-spectre-v2-分支目标注入)
    - [3. Meltdown和MDS](#3-meltdown和mds)
      - [3.1 Meltdown (CVE-2017-5754)](#31-meltdown-cve-2017-5754)
      - [3.2 微架构数据采样 (MDS)](#32-微架构数据采样-mds)
    - [4. 安全编程实践](#4-安全编程实践)
      - [4.1 恒定时间编程](#41-恒定时间编程)
      - [4.2 防止推测执行漏洞的模式](#42-防止推测执行漏洞的模式)
    - [5. 内存屏障和序列化](#5-内存屏障和序列化)
      - [5.1 屏障指令](#51-屏障指令)
      - [5.2 C11内存序与推测执行](#52-c11内存序与推测执行)
  - [⚠️ 常见陷阱](#️-常见陷阱)
    - [陷阱 SE01: 依赖边界检查后的访问](#陷阱-se01-依赖边界检查后的访问)
    - [陷阱 SE02: 忽略编译器优化](#陷阱-se02-忽略编译器优化)
    - [陷阱 SE03: 误解未定义行为](#陷阱-se03-误解未定义行为)
  - [✅ 质量验收清单](#-质量验收清单)
    - [5.3 编译器防护选项](#53-编译器防护选项)
    - [5.4 微码更新检测](#54-微码更新检测)
